...
def iter_snapshot_rows(settings: Any | None = None, redis_client: Any | None = None, chunk_size: int = 1000):
    """Yield raw Technical Snapshot JSONL rows from Redis when available, otherwise local mirror.

    Redis is the source of truth on Railway; the local file remains a mirror/fallback.
    """
    chunk_size = max(100, int(chunk_size or 1000))
    if redis_client is not None:
        try:
            total = int(redis_client.llen(SNAPSHOT_REDIS_RECORDS_KEY) or 0)
        except Exception:
            total = 0
        start = 0
        while start < total:
            end = min(total - 1, start + chunk_size - 1)
            rows = redis_client.lrange(SNAPSHOT_REDIS_RECORDS_KEY, start, end) or []
            for row in rows:
...
def write_snapshot_dataset_from_store(settings: Any | None = None, redis_client: Any | None = None, output_path: str | Path | None = None) -> dict:
    """Materialize Technical Snapshot records into a JSONL file for export."""
    path = Path(output_path or snapshot_path(settings))
    path.parent.mkdir(parents=True, exist_ok=True)
    count = 0
    with path.open("w", encoding="utf-8") as fh:
        for row in iter_snapshot_rows(settings=settings, redis_client=redis_client):
            fh.write(row.rstrip("\n") + "\n")
            count += 1
    return {"ok": True, "path": str(path), "records": count, "size_bytes": path.stat().st_size if path.exists() else 0}


def create_snapshot_dataset_zip(settings: Any | None = None, redis_client: Any | None = None, output_path: str | Path | None = None) -> dict:
    """Write live Technical Snapshot JSONL and compress it for Telegram/file download."""
    jsonl_result = write_snapshot_dataset_from_store(settings=settings, redis_client=redis_client, output_path=output_path or snapshot_path(settings))
...
def build_technical_dataset_export_file(settings: Any | None = None, redis_client: Any | None = None) -> dict:
    """Create a ZIP export for live Technical Snapshot data."""
    storage = snapshot_storage_status(settings, redis_client=redis_client)
    records = int(storage.get("redis_count") or 0)
    if records <= 0 and not snapshot_path(settings).exists():
        return {"ok": False, "message": "لا توجد داتا Live Snapshot للتصدير."}
    result = create_snapshot_dataset_zip(settings=settings, redis_client=redis_client, output_path=snapshot_path(settings))
    ...
    return {
        ...
        "caption": "\n".join([
            "📦 Live Technical Snapshot Dataset",
