...
def _localized_warning_notes(warnings, limit: int = 3) -> list[str]:
    ...
    The signal source may contain English rule names, Arabic fragments, or
    mojibake/broken Arabic. This function shows only clean user-facing notes.
    """
    warning_rules = [
        (
            (
    ...
    def _looks_broken_arabic(value: str) -> bool:
        text = str(value or "").strip()
        if not text:
            return True
        bad_tokens = ("Ø", "Ù", "Â", "Ã", "Ð", "Ñ", "�")
        if any(token in text for token in bad_tokens):
            return True
        # Known broken/unclear Arabic fragments that should never be shown raw.
        lower = text.lower()
        if any(token in lower for token in ("بره", "نحت", "تأد")):
            return True
        return False
    ...
    for raw in list(warnings or [])[: max(1, int(limit or 3))]:
        ...
        # First, hard-normalize known Arabic fragments that used to appear broken.
        if any(token in key for token in ("ارتداد", "مقاومة", "بره", "نحت", "تأد", "تاكد")):
            translated = "ارتداد قريب من المقاومة"
        elif any(token in key for token in ("اختراق", "ضعف")):
            translated = "ضعف تأكيد الاختراق"
        elif any(token in key for token in ("حجم", "سيولة", "تداول")):
            translated = "حجم التداول غير داعم"
        ...
        # Never show broken Arabic/raw mojibake. Either translate it or skip it.
        if not translated and _looks_broken_arabic(cleaned):
            continue

        # Do not show raw Arabic warnings unless they were normalized above.
        # This prevents unreadable fragments from leaking into Telegram.
        has_arabic = any("\u0600" <= ch <= "\u06FF" for ch in cleaned)
        if not translated and has_arabic:
            continue
