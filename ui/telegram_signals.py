...
def _simulation_badge_lines(execution_result: dict | None = None) -> list[str]:
    """Tiny visual badge for simulation-mode Telegram signal cards only."""
    if str((execution_result or {}).get("runtime_mode") or "").strip().lower() != "simulation":
        return []
...
def build_signal_message(signal: SignalCandidate, execution_result: dict | None = None) -> str:
    ...
    if is_execution:
        ...
        return "\n".join([*_simulation_badge_lines(execution_result), *lines])
