    ...
    try:
        ...
        for i, symbol in enumerate(symbols, start=1):
            ...
            try:
                ...
                append_log(f"Loaded {len(candles)} candles for {symbol}", redis_client)
            except Exception as exc:
                append_log(f"Failed loading {symbol}: {exc}", redis_client)
            status.update({
                "symbols_done": i,
                "progress_pct": round((i / max(1, len(symbols))) * 35.0, 2),
                "message": f"Loading candles: {i}/{len(symbols)}",
            })
            set_status(status, redis_client)
...
def start_replay(days: int = 45, symbols_limit: int = 200, timeframe: str = "15m", redis_client: Any | None = None, settings: Any | None = None) -> dict[str, Any]:
