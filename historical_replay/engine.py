...
def _btc_hourly_ma5_guard_from_15m(candles: list[HistoricalCandle], idx: int) -> dict:
    """Approximate BTC 1h MA5 pressure from replay 15m candles.

    The live guard uses BTC 1h candles. Replay data is 15m, so we sample every
    four 15m closes to get completed 1h closes. This keeps historical replay
    mode decisions aligned with the live MA5 pressure guard.
    """
    ...
    while j >= 0 and len(closes) < 10:
        try:
            close = float(candles[j].close or 0.0)
        except Exception:
            close = 0.0
        if close > 0:
            closes.append(close)
...
def _market_snapshot_at(symbol_candles: dict[str, list[HistoricalCandle]], idx: int) -> MarketSnapshot:
    changes: list[float] = []
    btc_change = 0.0
    btc_candles: list[HistoricalCandle] | None = None
    ...
    hourly_guard = _btc_hourly_ma5_guard_from_15m(btc_candles or [], idx)
    return MarketSnapshot(
        ...
        hourly_ma5_pressure=bool(hourly_guard.get("hourly_ma5_pressure", False)),
        btc_1h_close=float(hourly_guard.get("btc_1h_close", 0.0) or 0.0),
        btc_1h_ma5=float(hourly_guard.get("btc_1h_ma5", 0.0) or 0.0),
        btc_1h_ma5_gap_pct=float(hourly_guard.get("btc_1h_ma5_gap_pct", 0.0) or 0.0),
    )
