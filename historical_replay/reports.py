...
def build_historical_replay_help() -> str:
    return "\n".join([
        ...
        "/replay_start_90d",
        "↳ يشغل Replay آخر 90 يوم؛ مناسب للاعتماد لكنه يحتاج وقت تحميل أطول.",
        "/replay_status",
...
# ✅ FIX: إضافة weighted_count لحساب الـ average بشكل صحيح
def _group_stats() -> dict[str, Any]:
    return {
        ...
        "max_gain_sum": 0.0,
        "drawdown_sum": 0.0,
        "weighted_result_sum": 0.0,
        "weighted_count": 0,       # ✅ FIX: عداد منفصل للـ weighted results
        "weighted_winners": 0,
        "weighted_losers": 0,
    }


# ✅ FIX: increment weighted_count بشكل صحيح
def _update_group(group: dict[str, Any], rec: dict[str, Any]) -> None:
    ...
    if outcome.get("runner_active"):
        group["runner_active"] += 1
    if outcome.get("runner_closed"):
        group["runner_closed"] += 1
    try:
        group["max_gain_sum"] += float(outcome.get("max_gain_24h") or 0.0)
        group["drawdown_sum"] += float(outcome.get("max_drawdown_24h") or 0.0)
        # ✅ FIX: نعد weighted_count فقط لما يكون فيه weighted result فعلي
        weighted_val = outcome.get("weighted_trade_result_pct")
        if weighted_val is not None:
            weighted = float(weighted_val)
            group["weighted_result_sum"] += weighted
            group["weighted_count"] += 1          # ✅ FIX: بدل قسمة على outcomes
            if weighted > 0:
                group["weighted_winners"] += 1
            elif weighted < 0:
                group["weighted_losers"] += 1
    except Exception:
        pass


# ✅ FIX: استخدام weighted_count بدل outcomes في القسمة
def _format_group(name: str, group: dict[str, Any]) -> list[str]:
    outcomes = int(group.get("outcomes") or 0)
    weighted_count = int(group.get("weighted_count") or 0)   # ✅ FIX
    ...
    # ✅ FIX: القسمة على weighted_count مش outcomes
    avg_weighted = (float(group.get("weighted_result_sum") or 0.0) / weighted_count) if weighted_count else 0.0
    return [
        f"{name}: {group.get('total', 0)}",
        f"  TP1: {_pct(group.get('tp1', 0), outcomes)} | TP2: {_pct(group.get('tp2', 0), outcomes)} | SL before TP1: {_pct(group.get('sl', 0), outcomes)}",
        f"  First TP1/SL: {_pct(group.get('first_tp1', 0), outcomes)} / {_pct(group.get('first_sl', 0), outcomes)}",
        f"  Runner active/closed: {_pct(group.get('runner_active', 0), outcomes)} / {_pct(group.get('runner_closed', 0), outcomes)}",
        f"  Avg weighted result: {avg_weighted:.2f}% | W/L: {_pct(group.get('weighted_winners', 0), outcomes)} / {_pct(group.get('weighted_losers', 0), outcomes)}",
...
def build_replay_summary_report(settings=None, redis_client: Any | None = None) -> str:
    ...
    rows = [
        ...
        "",
        "🎯 TP/SL Lifecycle Outcomes",
        "Model: Smart SL 1.2%-2.8%, TP1=1.2R, TP2=2.0R, partial exits 40/40/20 or recovery 50/25/25",
    ]
    ...
    rows.extend(["", "ملاحظة: داخل نفس الشمعة يتم الحساب بتحفظ، الوقف/الحماية قبل الهدف لتجنب تضخيم النتائج."])
    return "\n".join(rows)
