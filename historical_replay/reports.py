...
def build_replay_start_report(settings=None, redis_client: Any | None = None, days: int = 45) -> str:
    result = start_replay(days=days, symbols_limit=int(getattr(settings, "scan_limit", 200) or 200), timeframe=str(getattr(settings, "timeframe", "15m") or "15m"), redis_client=redis_client)
    ...
    return "\n".join([
        ...
        "",
        "ملاحظة: تم فصل التحكم والتخزين الآن داخل historical_replay/. التشغيل الثقيل للشموع التاريخية يضاف كـ worker/job منفصل حتى لا يبطئ البوت الحي.",
    ])
...
def build_replay_summary_report(settings=None, redis_client: Any | None = None) -> str:
    ...
    return "\n".join([
        ...
        f"Blocked by limits: {status.get('blocked_by_limits', 0)}",
        "",
        "TP/SL outcomes and mode-gate statistics will appear after the heavy replay worker writes records.",
    ])
