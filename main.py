...
from analytics.gate_simulation import build_gate_sim_all_report, build_gate_sim_report
    ...
    for update in updates.get("result", []):
        ...
        for command in commands:
            ...
            if command == "/gate_sim_normal":
                _send_text(sender, build_gate_sim_report("normal", settings, redis_client=_snapshot_redis_client(trade_store)))
                continue
            if command == "/gate_sim_recovery":
                _send_text(sender, build_gate_sim_report("recovery", settings, redis_client=_snapshot_redis_client(trade_store)))
                continue
            if command == "/gate_sim_strong":
                _send_text(sender, build_gate_sim_report("strong", settings, redis_client=_snapshot_redis_client(trade_store)))
                continue
            if command == "/gate_sim_all":
                _send_text(sender, build_gate_sim_all_report(settings, redis_client=_snapshot_redis_client(trade_store)))
                continue
