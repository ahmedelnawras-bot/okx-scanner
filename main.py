...
TELEGRAM_COMMANDS = {
    "/help": "عرض الأوامر المختصرة والمنظمة",
    "/mood": "حالة السوق والمود الحالي",
    "/status": "حالة السوق (alias)",
    "/market": "حالة السوق (alias)",
    "/market_status": "حالة السوق (alias)",
    "/how_it_work": "شرح طريقة عمل البوت",

    "/report_1h": "تقرير آخر ساعة",
    "/report_today": "تقرير اليوم الأساسي",
    "/report_7d": "تقرير آخر 7 أيام",
    "/report_30d": "تقرير آخر 30 يوم",
    "/report_all": "كل الصفقات",
    "/report_deep": "تحليل متقدم شامل",

    "/report_exits": "تحليل جودة الخروج TP1/TP2/SL",
    "/report_setups": "تحليل الأداء حسب نوع الإشارة",
    "/report_market": "تحليل الأداء حسب حالة السوق",
    "/report_losses": "تحليل أسباب الخسارة",

    "/stats_since_reset": "الأداء منذ آخر تصفير",
    "/reset_stats": "تصفير نتائج اللونج",
}
...
ℹ️ <b>ملاحظات:</b>
• البوت مخصص لإشارات Long Futures.
• الفريم الأساسي 15m مع تأكيد 1H.
• كل إشارة بها زر 📌 Track للمتابعة.
• /mood يوضح هل السوق يسمح باللونج العادي أو القوي فقط أو يمنع اللونج.
• التقارير المالية تقديرية حسب إعدادات إدارة رأس المال في performance.py.

🔥 <b>اقتراح:</b>
ابدأ دائمًا بـ /mood ثم /report_today."""
...
def build_market_status_message() -> str:
    ...
    """
    try:
        snapshot = load_market_status_snapshot(max_age_seconds=240)

        if snapshot:
            # بناء الرسالة من الـ Snapshot
            created_ts = int(snapshot.get("created_ts", 0))
            ...
            # حساب المود المقترح بدون كتابة Redis
            suggested_mode_result = determine_long_market_mode(
            ...
            suggested_mode = suggested_mode_result.get("mode", current_mode)
            suggested_reason = suggested_mode_result.get("reason", "")
            data_source = "⚡ البيانات من Snapshot محفوظ"
        else:
            # Live calculation للأسف سيكون ثقيلًا
            ...
            age_seconds = 0
            age_str = "الآن (Live)"
            data_source = "⚠️ البيانات Live لأن Snapshot غير متاح"
        ...
        if current_mode == MODE_NORMAL_LONG:
            ...
            action = "راقب السوق قبل الدخول."

        lines = [
            "🧭 <b>Market Mood - LONG</b>",
            "",
            f"🤖 <b>المود الحالي:</b> {mode_ar}",
            f"🧮 <b>المود المحسوب الآن:</b> {suggested_mode_ar}",
            f"🧠 <b>سبب الحساب:</b> {html.escape(suggested_reason)}",
            f"{data_source}",
        ]
        ...
        if suggested_mode != current_mode:
            lines.append("")
            lines.append("⚠️ المود المحسوب مختلف عن المخزن، وسيتحدث مع دورة الفحص القادمة.")
        ...
        if current_mode == MODE_BLOCK_LONGS:
            lines.append("السوق غير مناسب للونج الآن. انتظار أفضل من مطاردة ارتداد ضعيف.")
        elif current_mode == MODE_STRONG_LONG_ONLY:
            lines.append("السوق يسمح بفرص محدودة فقط. ركّز على الجودة وليس عدد الإشارات.")
        elif current_mode == MODE_RECOVERY_LONG:
            lines.append("الوضع Recovery: احتمالية الارتداد موجودة لكن المخاطرة أعلى.")
        else:
            lines.append("الوضع طبيعي نسبيًا، لكن راجع كل إشارة على الشارت قبل الدخول.")
...
def detect_late_pump_long(
    ...
    try:
        ...
        checks = sum([over_ma, hot_rsi, pump_volume, big_candle, fast_4h_move,
                      (vwap_distance >= 2.4), (rsi_slope <= 0 and rsi_now >= 65),
                      (macd_hist_slope < 0)])
        ...
        if bull_continuation_risk and late_pump_risk:
            should_block = True
        # Adjusted extreme late pump to protect strong breakouts
...
def sort_reasons(reasons):
    priority = {
        ...
        # newly added warnings after late pump
        "خطر نهاية الزخم بعد Pump": 115,
        "بعيد عن VWAP": 116,
        "RSI بدأ يضعف": 117,
        "زخم MACD يتراجع": 118,
        "MACD سلبي": 119,
...
💼 <b>حجم الصفقة:</b>
إجمالي الحجم: {RECOVERY_TOTAL_SIZE_PCT}% من الصفقة العادية
...
🎯 <b>الأهداف:</b>
TP1: {fmt_num(tp1, 6)} ({fmt_pct(tp1_pct)} | {rtl_fix(f"{rr1}R")})
TP2: {fmt_num(tp2, 6)} ({fmt_pct(tp2_pct)} | {rtl_fix(f"{rr2}R")})
...
# =========================
# MARKET GUARD (original, used for mode determination only)
# =========================
...
def get_market_guard_snapshot(ranked_pairs, btc_mode: str, alt_snapshot: dict) -> dict:
    ...
    try:
        ...
        for item in sample:
            ...
            candles = get_candles(
                symbol,
                MARKET_GUARD_TIMEFRAME,
                MARKET_GUARD_CANDLE_LIMIT
            )
            df = to_dataframe(candles)
            if df is None or df.empty:
                continue
        ...
        if valid < MARKET_GUARD_MIN_VALID:
            return {
                ...
                "avg_change_15m": 0.0,
                "btc_change_15m": 0.0,
                "reason": f"valid pairs too low ({valid})",
            }
        ...
        if red_ratio >= MARKET_GUARD_RED_RATIO_BLOCK and avg_change <= MARKET_GUARD_AVG_CHANGE_15M_BLOCK:
            block = True
            reason_parts.append(
                f"red_ratio={red_ratio:.2f}>={MARKET_GUARD_RED_RATIO_BLOCK:.2f} "
                f"and avg_change={avg_change:.2f}<={MARKET_GUARD_AVG_CHANGE_15M_BLOCK:.2f}"
            )
        if btc_change <= MARKET_GUARD_BTC_CHANGE_15M_BLOCK and red_ratio >= 0.55:
            block = True
            reason_parts.append(
                f"btc_change={btc_change:.2f}<={MARKET_GUARD_BTC_CHANGE_15M_BLOCK:.2f} "
                f"and red_ratio={red_ratio:.2f}>=0.55"
            )
        if MARKET_GUARD_ALT_WEAK_BLOCK and alt_mode in ("🔴 ضعيف",) and red_ratio >= 0.60:
            block = True
            reason_parts.append(
                f"alt_mode={alt_mode} and red_ratio={red_ratio:.2f}>=0.60"
            )
...
# =========================
# DETERMINE LONG MARKET MODE (REVISED WITH allow_state_writes)
# =========================
def determine_long_market_mode(
    ...
    if current_mode == MODE_STRONG_LONG_ONLY:
        ...
        weak_market = (
            ...
            or (avg_change < -0.30)
            or (btc_mode in ("🟡 محايد", "🔴 هابط") and alt_mode != "🟢 قوي")
        )
        if weak_market:
            if allow_state_writes and r:
                try:
            ...
            if now_ts - normal_candidate_since >= NORMAL_CANDIDATE_DURATION:
                if allow_state_writes and r:
                    try:
                        ...
                        pass
                return {"mode": MODE_NORMAL_LONG, "reason": "الريكافري استقر بما يكفي، رجوع للوضع الطبيعي"}
...
def format_mode_transition_message(old_mode: str, new_mode: str, reason: str = "") -> str:
    ...
    if new_mode == MODE_NORMAL_LONG:
        ...
        lines = [
            ...
            "",
            "حجم الصفقة:",
            f"• إجمالي الصفقة = {RECOVERY_TOTAL_SIZE_PCT}% من الحجم الطبيعي",
        ...
        try:
            ...
            for pair_data in filtered_scan_pairs:
                ...
                is_reverse = is_oversold_reversal_long(
                    df=df, dist_ma=dist_ma, change_24h=change_24h,
                    vol_ratio=vol_ratio, funding=funding,
                )
                signal_row = get_signal_row(df)
                if signal_row is None:
                ...
                rsi_slope = get_rsi_slope(df, bars=3)
                macd_hist = _safe_float(signal_row.get("macd_hist"), 0.0)
                macd_hist_slope = get_macd_hist_slope(df, bars=3)
                ...
                if current_mode == MODE_RECOVERY_LONG:
                    ...
                    if candle_range > 0:
                        ...
                        close_position = 0.5
                    if not (close > open_ or close_position >= 0.58):
                        continue
                    if macd_hist_slope < 0 and rsi_slope < -1.0:
                        continue
                    price = close
                    ...
                    entry1 = price
                    entry2 = round(entry1 - atr_value * RECOVERY_ENTRY2_ATR_MULT, 6)
                    avg_entry = round((entry1 + entry2) / 2, 6)
                    sl = round(avg_entry - atr_value * RECOVERY_SL_ATR_MULT, 6)
                    min_sl = round(avg_entry * 0.955, 6)
                    max_sl = round(avg_entry * 0.92, 6)
                    sl = max(max_sl, min(min_sl, sl))
                    rr1, rr2 = get_rr_targets_long(signal_type="standard", entry_timing="")
                    ...
                    msg = build_recovery_long_message(
                        symbol=symbol, entry1=entry1, entry2=entry2,
                    ...
                    if (
                        ...
                        if locked:
                            ...
                            if sent_data.get("ok"):
                                ...
                                last_candle_cache[symbol] = get_signal_candle_time(df)
                                alert_snapshot = {
                                    "alert_id": alert_id,
                                    "symbol": symbol,
                                    "timeframe": TIMEFRAME,
                                ...
                                safe_register_trade(
                                    ...
                                    btc_mode=btc_mode,
                                    funding_label="🟡 محايد",
                                    reasons=["RECOVERY_LONG", "POST_CRASH_REBOUND"],
                                    pre_breakout=False,
                                    breakout=False,
                                    vol_ratio=vol_ratio,
                                    ...
                                    news_titles=[],
                                    warning_reasons=["Recovery عالي المخاطر", "حجم الصفقة 30% فقط", "دخول مقسم 15% + 15%"],
                                    pullback_low=entry2,
                                    pullback_high=entry1,
                                    pullback_entry=entry2,
                                    rr1=rr1,
                                ...
                                logger.info(f"SENT RECOVERY LONG → {symbol} | avg_entry={avg_entry}")
                            else:
                                release_signal_slot(symbol, get_signal_candle_time(df), "long")
                                logger.error(f"FAILED SEND RECOVERY → {symbol}")
                    continue

                # --- NORMAL / STRONG_LONG_ONLY FILTERS ---
                if rsi_now > 75 and not is_reverse:
                    logger.info(f"{symbol} → skipped (RSI > 75 extreme peak)")
                    continue
                if rsi_now > 70 and dist_ma > 4 and not is_reverse:
                ...
                if above_upper_bb and rsi_now >= 68 and dist_ma >= 3.8 and not is_reverse:
                    if breakout_quality != "strong":
                        logger.info(
                            f"{symbol} → skipped (upper BB peak guard | "
                            f"rsi={rsi_now:.1f} | dist_ma={dist_ma:.2f} | "
                            f"breakout_quality={breakout_quality})"
                        )
                        continue
                ...
                late_guard = detect_late_pump_long(
                    ...
                    dist_ma=dist_ma, rsi_now=rsi_now, vol_ratio=vol_ratio,
                    candle_strength=candle_strength, breakout=breakout,
                    pre_breakout=pre_breakout, breakout_quality=breakout_quality,
                    change_4h=change_4h,
                ...
                # Allow strong breakout to proceed even if late guard would block
                ...
                if not is_reverse and vwap_distance >= 3.0 and not pre_breakout:
                    if not breakout or breakout_quality != "strong":
                        continue

                # Bull Market VWAP Filter
                ...
                # RSI Slope Weakness Filter (protect strong breakout)
                if not is_reverse:
                    if (
                        rsi_now >= 67
                        and rsi_slope <= 0
                ...
                # MACD Histogram Filter (protect strong breakout)
                if not is_reverse:
                ...
                if entry_maturity_data.get("block_signal"):
                    if not is_reverse and not pre_breakout and not (breakout_quality == "strong" and mtf_confirmed and vol_ratio >= 1.8):
                ...
                # Apply entry maturity adjustments
                if not is_reverse:
                    effective_score -= entry_maturity_data.get("maturity_penalty", 0.0)
                    effective_score += entry_maturity_data.get("maturity_bonus", 0.0)

                # Add entry maturity warnings
                ...
                dynamic_threshold = get_dynamic_entry_threshold(
                    market_state=market_state, score_result=score_result,
                    vol_ratio=vol_ratio, mtf_confirmed=mtf_confirmed,
                    is_new=is_new, gaining_strength=gaining_strength,
                )
                ...
                if score_result["score"] < dynamic_threshold and early_priority != "strong":
                    logger.info(f"{symbol} → rejected (score<{dynamic_threshold})")
                    continue
                ...
                # Bull Market Continuation Smart Filter
                if (
                    market_state == "bull_market"
                ...
                momentum_priority = get_momentum_priority(
                    score=float(score_result["score"]),
                    breakout=breakout, vol_ratio=vol_ratio, is_new=is_new,
                    pre_breakout=pre_breakout, dist_ma=dist_ma,
                    gaining_strength=gaining_strength, early_priority=early_priority,
                    is_reverse=is_reverse,
                )
                ...
                if (setup_type in WEAK_SETUP_TYPES
                    ...
                    if effective_score < 7.4:
                        logger.info(f"{symbol} → skipped (weak historical setup, score dropped to {effective_score})")
                        continue

                setup_stats = get_setup_type_stats(
                    redis_client=r, market_type="futures", side="long",
                    setup_type=setup_type, since_ts=stats_reset_ts,
                )
                ...
                candidate = {
                    ...
                    # New metrics stored for diagnostics
                    "vwap_distance": vwap_distance,
                    "rsi_slope": rsi_slope,
                    "macd_hist": macd_hist,
                    "macd_hist_slope": macd_hist_slope,
                    ...
                    "message": build_message(
                        ...
                        btc_mode=btc_mode, btc_dominance_proxy=btc_dominance_proxy,
                        tv_link=tv_link, is_new=is_new, change_24h=change_24h,
                        market_state_label=market_state_label,
                        market_bias_label=market_bias_label,
                        alt_mode=alt_mode, news_warning=news_warning_text,
                        opportunity_type=opportunity_type,
                        entry_timing=entry_timing, display_risk=display_risk,
                        setup_stats=setup_stats, is_reverse=is_reverse,
                        reversal_4h_confirmed=reversal_4h_result.get("confirmed", False),
            ...
            for candidate in top_candidates:
                ...
                if sent_data.get("ok"):
                    ...
                    safe_register_trade(
                        ...
                        rr1=candidate.get("rr1", 1.5),
                        rr2=candidate.get("rr2", 3.0),
                    )
                    logger.info(
                        f"SENT LONG → {symbol} | score: {candidate['score']} | "
                        f"mode: {current_mode} | alert_id={candidate['alert_id']}"
                    )
                else:
