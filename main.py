...
def build_execution_report_message(period: str = "all") -> str:
    ...
    try:
        ...
        lines = [
            ...
            f"✅ <b>إجمالي المرشحين:</b> {len(trades)} | 🟢 <b>مفتوحة:</b> {len(open_trades)} | 🏁 <b>مغلقة:</b> {len(closed_trades)}",
            f"🟢 <b>رابحة:</b> {len(winners_pairs)} | 🔴 <b>خاسرة:</b> {len(losers_pairs)}",
            "━━━━━━━━━━━━",
            "💰 <b>Wallet Impact</b>",
            f"📌 <b>رأس المال:</b> {wallet_capital_usd:.0f}$",
            f"✅ <b>المحقق</b> | 📈 {money(realized_profit_usd)} | {_pct_safe(realized_profit_pct)}",
            f"📉 {money(realized_loss_usd)} | {_pct_safe(realized_loss_pct)}",
            f"⚖️ {money(realized_net_usd)} | {_pct_safe(realized_net_pct)}",
            f"🔄 <b>المفتوحة</b> | 📈 {money(floating_profit_usd)} | {_pct_safe(floating_profit_pct)}",
            ...
            "━━━━━━━━━━━━",
            "🧠 <b>Execution Behavior Summary</b> <b>(40/40/20)</b>",
            f"📈 <b>Avg Win:</b> {_pct_safe(avg_winner)}",
            f"📉 <b>Avg Loss:</b> {_pct_safe(avg_loser)}",
            f"🎯 <b>TP1:</b> {tp1_hits / total * 100:.1f}% | 🏁 <b>TP2:</b> {tp2_hits / total * 100:.1f}%",
            f"🔁 <b>TP1→TP2:</b> {tp1_to_tp2_pct:.1f}%",
            f"🔄 <b>Trail:</b> {trailing_wins / total * 100:.1f}% | 🔒 <b>BE:</b> {breakeven_exits / total * 100:.1f}% | 🛑 <b>SL:</b> {direct_sl / total * 100:.1f}%",
            f"⚡ <b>Avg Exp Profit:</b> {exposure(avg_open_floating)}",
            f"💡 <b>جودة العائد مقابل المخاطرة:</b> {rr_quality}",
            "━━━━━━━━━━━━",
