...
def is_breakout(df, lookback=20):
    """
    Raw breakout detector فقط.
    لا نعتمد عليه وحده كاختراق نهائي — التحقق الكامل يحصل داخل calculate_long_score.
    """
    try:
...
def calculate_long_score(
    ...
    try:
        ...
        if idx > 20:
            ...
            if recent_high > 0:
                breakout_extension = ((close - recent_high) / recent_high) * 100
    except Exception:
        breakout_extension = 0.0

    validated_breakout = bool(
        breakout
        and vol_ratio >= 1.20
        and candle_strength >= 0.45
        and not exhaustion_wick
        and dist_ma <= 3.8
    )
    ...
    strong_breakout = bool(
        ...
        and vol_ratio >= 1.60
        and candle_strength >= 0.45
        and breakout_extension <= 2.2
    )

    late_breakout = bool(
        breakout
        and (dist_ma > 4.2 or breakout_extension > 3.5)
    )
    ...
    if not consolidation_ok:
        score -= 0.6
        warning_reasons.append("حركة سريعة بدون تحضير واضح")

    # ===================== BREAKOUT / PRE-BREAKOUT =====================
    if validated_breakout:
        score += 1.4
        reasons.append("اختراق")
    elif breakout and not validated_breakout:
        score -= 0.7
        warning_reasons.append("اختراق ضعيف")
    ...
    if validated_breakout:
        if breakout_extension <= 1.8:
            score += 0.6
            reasons.append("اختراق مبكر")
        elif 1.8 < breakout_extension <= 3.0:
            score += 0.2
        elif breakout_extension > 3.5:
            score -= 0.7
    ...
    if breakout and not consolidation_ok:
        score -= 0.5
        warning_reasons.append("اختراق بدون تحضير كافٍ")

    if market_state == "bull_market" and late_breakout:
        score -= 0.5
        warning_reasons.append("اختراق متأخر داخل حركة صاعدة ممتدة")

    if score >= 8.8:
        if not ((validated_breakout or pre_breakout) and mtf_confirmed and vol_ratio >= 1.5):
            score -= 0.8
    ...
    if not consolidation_ok and not breakout and not pre_breakout and vol_ratio < 1.35:
        fake_signal = True

    if breakout and not validated_breakout and not pre_breakout:
        fake_signal = True

    if close <= ma * 0.997 and not breakout and not pre_breakout and not is_reverse:
        fake_signal = True

    if rsi < 46 and not breakout and not pre_breakout and not is_reverse:
        fake_signal = True

    if score >= 8.5 and vol_ratio < 1.15:
        fake_signal = True

    if market_state == "btc_leading" and not validated_breakout and not pre_breakout:
        if score < 7.3:
            fake_signal = True
    elif market_state == "risk_off" and score < 7.6:
        fake_signal = True
    elif btc_dominance_proxy == "🔴 ضد الألت" and not validated_breakout and not pre_breakout:
        if score < 7.3:
            fake_signal = True
