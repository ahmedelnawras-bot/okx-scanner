...
def _smart_target_profile(pair: PairCandidate, setup_type: str, entry_timing: str, market_mode: str) -> dict[str, float | str]:
    """Return the active smart TP/SL profile.

    The trade management model remains unchanged downstream:
    - Normal/Strong: 40% TP1, 40% TP2, 20% runner.
    - Recovery: 50% TP1, 25% TP2, 25% runner.

    This function only improves the placement of SL/TP levels.  With the
    current lightweight scanner inputs we do not have full structure/ATR yet,
    so the risk distance is a conservative volatility proxy clamped to the
    agreed execution range: 1.2% - 2.8%.
    """
    tags = set(pair.tags or [])
    change_abs = abs(float(pair.change_pct or 0.0))
    turnover = float(pair.turnover_usdt or 0.0)

    # Start wider than the old fixed-percent model because the intended
    # execution size is small and crypto swaps often need breathing room.
    ...
    if "near_resistance" in tags:
        # Do not reward a late/chasing entry with excessive room. The execution
        # gate can still downgrade it; TP/SL stays controlled here.
        risk_pct -= 0.10
...
def _infer_quality_context(pair: PairCandidate, setup_type: str, entry_timing: str, score: float, market_mode: str, warnings: list[str]) -> dict:
    ...
    # Recovery relative bounce proxy: compare symbol bounce/change with BTC bounce/change.
    # In the lightweight rebuild this uses available ticker percentage proxy. The full
    # old engine can later replace it with true low-to-current 15m bounce.
