...
def process_trade_candidate(
    ...
    # ─────────────────────────────────────────
    # Trend Block Guard
    # لو البلوك سببه ترند هابط مؤكد (TREND) → نرفض مسار block_exception
    # بالكامل. الترند الهابط المستمر مش وقت استثناءات (صيد سكين، 8% WR).
    # الـ FLASH block (انهيار سريع) يفضل ليه استثناءاته لأنه ممكن يرتد.
    # ─────────────────────────────────────────
    if path == "block_exception" and str(block_type).upper() == "TREND":
        return _base_response(
            signal=signal,
            gate=gate,
            status="rejected_risk",
            reason="trend_block_no_exceptions",
            extra={"block_type": "TREND", "block_exception_suppressed": True},
            decision_trace_id=decision_trace_id,
            effective_risk_mode=risk_mode,
        )
