...
def behavior_summary_lines(
    ...
    *,
    ...
    # ✅ FIX #10: نِسَب الخروج أساسها الصفقات المغلقة فقط (مش المفتوحة معاها).
    ...
    # Milestones (وصول — قد تتداخل مع أسباب الخروج، ليست buckets جامعة)
    tp1_count = sum(1 for t in closed if t.tp1_hit)
    tp2_count = sum(1 for t in closed if t.tp2_hit)
    tp1_rate = tp1_count / total_closed * 100.0
