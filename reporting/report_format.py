...
def tradingview_symbol(symbol: str) -> str:
    base = symbol.replace("-USDT-SWAP", "USDT").replace("-", "")
    return f"OKX:{base}.P"


def tradingview_url(symbol: str) -> str:
    return f"https://www.tradingview.com/chart/?symbol={tradingview_symbol(symbol)}"


def trade_duration(t: TrackedTrade) -> str:
    start = _ensure_aware(getattr(t, "opened_at", None)) or _ensure_aware(getattr(t, "updated_at", None))
    end = _ensure_aware(getattr(t, "closed_at", None)) or _ensure_aware(getattr(t, "updated_at", None)) or datetime.now(timezone.utc)
    if not start:
        return "n/a"
...
def trade_card_lines(t: TrackedTrade, *, exposure_label: bool = True) -> list[str]:
    ...
    stage = trade_stage(t)
    stage_text = " | ".join([stage, *extra]) if extra else stage
    tv_symbol = tradingview_symbol(t.symbol)
    return [
        f"• <b>{t.symbol}</b> | {pnl_label}",
        f"⏱ {trade_duration(t)} | {stage_text} | ⭐ {float(t.score or 0):.2f}",
        f"TP1: {float(t.tp1 or 0):.6f} | TP2: {float(t.tp2 or 0):.6f}",
        f"SL: {float(t.sl or 0):.6f} | Plan: {float(getattr(t, 'tp1_close_pct', 40.0) or 40.0):.0f}/{float(getattr(t, 'tp2_close_pct', 40.0) or 40.0):.0f}/{float(getattr(t, 'runner_close_pct', 20.0) or 20.0):.0f}",
        f"{clean_setup(t)}",
        f'<a href="{tradingview_url(t.symbol)}">🔗 TradingView: {tv_symbol}</a>',
    ]
...
def quick_stats_lines(trades: list[TrackedTrade], *, label: str = "Quick Stats", item_name: str = "Trades") -> list[str]:
    """Old-report style quick stats.

    Win Rate is calculated from closed winners/losses only. Open trades,
    rejected checks, candidate-only checks, and breakeven rows are excluded.
    """
    opened = open_trades(trades)
    closed = closed_trades(trades)
    ...
    win_rate = len(winners) / denom * 100.0 if denom else 0.0
    return [
        f"📊 <b>{label}</b>",
...
def period_cutoff(period: str, now: datetime | None = None) -> datetime | None:
...
def trade_activity_time(t: TrackedTrade) -> datetime | None:
    return _ensure_aware(t.closed_at or t.updated_at or t.opened_at)


def filter_trades_by_period(trades: list[TrackedTrade], period: str) -> list[TrackedTrade]:
    cutoff = period_cutoff(period)
    if cutoff is None:
        return list(trades)
    return [t for t in trades if (trade_activity_time(t) or datetime.min.replace(tzinfo=timezone.utc)) >= cutoff]


def check_time(item: dict) -> datetime | None:
    for key in ("ts", "created_at", "time", "timestamp", "updated_at"):
...
def filter_checks_by_period(items: list[dict], period: str) -> list[dict]:
    cutoff = period_cutoff(period)
    if cutoff is None:
        return list(items)
    return [x for x in items if (check_time(x) or datetime.min.replace(tzinfo=timezone.utc)) >= cutoff]
