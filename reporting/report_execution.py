...
def build_execution_period_table(
    ...
    # v135: keep period reports useful on mobile too; add the same compact samples
    # used by the full execution report, scoped to the requested period.
    opened = open_trades(trades)
    closed_items = closed_trades(trades)
    open_winners = sorted([t for t in opened if trade_effective_pnl(t) >= 0], key=trade_effective_pnl, reverse=True)
    open_losers = sorted([t for t in opened if trade_effective_pnl(t) < 0], key=trade_effective_pnl)
    closed_winners = sorted([t for t in closed_items if trade_effective_pnl(t) > 0], key=trade_effective_pnl, reverse=True)
    closed_losers = sorted([t for t in closed_items if trade_effective_pnl(t) < 0], key=trade_effective_pnl)
    append_trade_cards(lines, "🟢 <b>Open Winners — Top 3</b>", open_winners[:3], limit=3)
    append_trade_cards(lines, "🔴 <b>Open Losers — Top 3</b>", open_losers[:3], limit=3)
    append_trade_cards(lines, "🏆 <b>Closed Winners — Top 3</b>", closed_winners[:3], limit=3)
    append_trade_cards(lines, "💀 <b>Closed Losers — Top 3</b>", closed_losers[:3], limit=3)
    return "\n".join(lines)
...
def build_execution_report(
    ...
    closed_winners = sorted([t for t in closed if trade_effective_pnl(t) > 0], key=trade_effective_pnl, reverse=True)
    closed_losers = sorted([t for t in closed if trade_effective_pnl(t) < 0], key=trade_effective_pnl)
