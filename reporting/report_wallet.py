...
def build_wallet_report(
    ...
    title: str = "💼 Wallet Impact — Execution",
    period: str = "since_start",
) -> str:
    # Wallet Impact is execution-only. Normal trades must not call this report.
    lines = [
        title,
        f"📅 {period_label(period)}" if period != "since_start" else "📅 Snapshot",
        SEP,
        "⚡ Scope: Execution accepted/tracked trades only",
    ...
    rows = all_rows if period == "since_start" else [(period_label(period), period)]

    lines.append("<code>Period       Closed$   Open$   Total$")
    for label, row_period in rows:
        subset = filter_trades_by_period(trades or [], row_period)
