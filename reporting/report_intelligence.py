...
from reporting.report_format import SEP, behavior_summary_lines, filter_checks_by_period, filter_trades_by_period, period_label, trade_effective_pnl


def build_intelligence_report(trades: list[TrackedTrade], title: str, period: str = "since_start") -> str:
    ...
    lines: list[str] = [title, f"📅 {period_label(period)}", SEP]
    lines.extend([
...
def build_execution_intelligence_report(
    trades: list[TrackedTrade],
    execution_results: list[dict],
    title: str = "🧠🚀 ذكاء صفقات التنفيذ",
    period: str = "since_start",
) -> str:
    execution_results = filter_checks_by_period(execution_results or [], period)
    base = build_intelligence_report(trades, title, period=period)
